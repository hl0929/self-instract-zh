...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=list,
        default=["data/generations/machine_generated_instances_zh.jsonl"],
        help="instance list"
    )
    parser.add_argument(
        ...
        nargs="+",
        type=list,
        default=["data/generations/is_clf_or_not_davinci-002_template_cls.jsonl"],
        help="classification_type_files list"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="data/finetuning/batch_241021",
        help="结果保存目录"
    )
    parser.add_argument(
        "--include-seed-tasks",
        action="store_true",
        help="是否在调优数据中包含人工编写的种子实例"
    )
    parser.add_argument(
        "--seed-tasks-path",
        type=str,
...
def main():
    ...
    classification_type_files = args.classification_type_files
    output_dir = args.output_dir
    include_seed_tasks = args.include_seed_tasks
    ...
    if include_seed_tasks:
        ...
        for task in seed_tasks:
            for instance in task["instances_zh"]:
                training_instances.append(
                    (task["instruction_zh"], instance["input_zh"], instance["output_zh"])
                )
